...
#define MAX_NAME_LENGTH 1024	//[haipe]增加名称 2018-12-4 以便回调，调整到这里来
...
struct AddFunctionInfo {
    ...
    void* param;
    char name[MAX_NAME_LENGTH];	//[haipe]增加名称 2018-12-4 以便回调
};
...
public:
    ...
    void set(const char* name, wkeJsNativeFunction getter, void* getterParam, wkeJsNativeFunction setter, void* setterParam) 
    {
        ...
        strncpy(this->name, name, MAX_NAME_LENGTH - 1); //[haipe]增加名称 2018-12-4 以便回调
    }
...
//#define MAX_NAME_LENGTH 1024      ////[haipe]增加名称 2018-12-4 以便回调，调整到前面了
...
jsValue WKE_CALL_TYPE js_outputMsg(const char* fname, jsExecState es, void* param) //[haipe]增加名称 2018-12-4 以便回调
{
...
jsValue WKE_CALL_TYPE js_getWebViewName(const char* fname, jsExecState es, void* param)  //[haipe]增加名称 2018-12-4 以便回调
{
    wkeWebView webView = jsGetWebView(es);
    return jsString(es, webView->name());
}

jsValue WKE_CALL_TYPE js_setWebViewName(const char* fname, jsExecState es, void* param) //[haipe]增加名称 2018-12-4 以便回调
{
